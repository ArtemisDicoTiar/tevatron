...
def main():
    ...
    if args.faiss:
        faiss.write_index(retriever.index, args.faiss)
        faiss_path = Path(args.faiss)
        didx2did = dict(enumerate(look_up))
        did2didx = {
            v: k
            for k, v in didx2did.items()
        }
        with (faiss_path.parent / "did2didx.json").open("w") as f:
            f.write(json.dumps(did2didx))
