...
from transformers import LlamaModel, PreTrainedModel, AutoModelForCausalLM
import logging
from peft import LoraConfig, get_peft_model, PeftModel, TaskType, AutoPeftModelForCausalLM, \
    AutoPeftModelForFeatureExtraction, AutoPeftModel
...
class RepLLaMA(EncoderModel):
    ...
    def gradient_checkpointing_enable(self, *args, **kwargs):
        self.lm_q.base_model.gradient_checkpointing_enable(*args, **kwargs)
        self.lm_p.base_model.gradient_checkpointing_enable(*args, **kwargs)
    ...
    def build(
        ...
        base_model = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path,
                                                          attn_implementation="flash_attention_2", **hf_kwargs)
    ...
    def load(
        ...
        hf_model = PeftModel.from_pretrained(base_model, model_name_or_path,
                                             is_trainable=True, config=config, adapter_name="adapter_model")
        # hf_model = AutoPeftModelForFeatureExtraction.from_pretrained(model_name_or_path)
        hf_model = hf_model.merge_and_unload()
        if hf_model.config.pad_token_id is None:
            hf_model.config.pad_token_id = 0
        model = cls(
            lm_q=hf_model,
            lm_p=hf_model,
            pooler=None,
